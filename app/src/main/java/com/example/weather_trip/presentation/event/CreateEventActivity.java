...
public class CreateEventActivity extends BaseActivity {
    ...
    private TextView tvWeatherIcon, tvWeatherTemp, tvWeatherDesc, tvWeatherHumidity, tvWeatherAdvice, tvWeatherError;
    ...
    private void setupReminderSpinner() {
        ...
        spinnerReminder.setSelection(2); // Default to "1 giờ"
    }
    ...
    private void fetchWeather(String city) {
        ...
        long diffDays = (selectedDate.getTimeInMillis() - System.currentTimeMillis()) / (24 * 60 * 60 * 1000);

        if (diffDays >= 0 && diffDays <= 5) {
            weatherRepository.getForecastByCity(city, new WeatherRepository.ForecastCallback() {
                @Override
                public void onSuccess(ForecastResponse forecast) {
                    int targetDate = (int) (selectedDate.getTimeInMillis() / 1000 / 86400);
                    Weather weather = forecast.getForecastForDate(targetDate);
                    mainHandler.post(() -> {
                        ...
                        if (weather != null) {
                            ...
                            fetchCurrentWeatherFallback(city);
                        }
                    });
                }
    ...
    private void fetchCurrentWeatherFallback(String city) {
    ...
    private void saveEvent() {
        ...
        new Thread(() -> {
            ...
            event.setUserId(sessionManager.getUserId());
            event.setTitle(title);
            event.setDescription(etDescription.getText().toString().trim());
            event.setLocationName(location);

            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd", Locale.US);
            event.setEventDate(Integer.parseInt(sdf.format(selectedDate.getTime())));
            event.setEventTime(etTime.getText().toString());
            event.setEventDatetime(selectedDate.getTimeInMillis());

            int selectedIndex = spinnerReminder.getSelectedItemPosition();
            event.setReminderMinutes(reminderMinutes[selectedIndex]);

            if (currentWeather != null) {
                event.setWeatherId(currentWeather.getWeatherId());
                event.setWeatherMain(currentWeather.getMain());
                event.setWeatherDescription(currentWeather.getDescription());
                event.setWeatherIcon(currentWeather.getIcon());
                event.setTemperature(currentWeather.getTemp());
                event.setTemperatureMin(currentWeather.getTempMin());
                event.setTemperatureMax(currentWeather.getTempMax());
                event.setHumidity(currentWeather.getHumidity());
                event.setWindSpeed(currentWeather.getWindSpeed());
                event.setClouds(currentWeather.getClouds());
                event.setWeatherCityName(currentWeather.getCityName());
                event.setWeatherTimestamp(currentWeather.getTimestamp());
            }

            long eventId = eventRepository.createEvent(event);

            if (eventId > 0) {
                reminderScheduler.scheduleReminder(eventId, event.getEventDatetime(), event.getReminderMinutes(), title);
            }
